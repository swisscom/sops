...
def main():
    ...
    argparser.add_argument("file", help="file to edit; create it if it doesn't exist")
    argparser.add_argument(
        "-k", "--kms", dest="kmsarn", help="comma separated list of KMS ARNs"
    )
    argparser.add_argument(
        "-p", "--pgp", dest="pgpfp", help="comma separated list of PGP fingerprints"
    )
    argparser.add_argument(
        "-d",
        "--decrypt",
    ...
    argparser.add_argument(
        "--input-type",
        dest="input_type",
        help="input type (yaml, json, ...), " "if undef, use file extension",
    )
    argparser.add_argument(
        "--output-type",
        dest="output_type",
        help="output type (yaml, json, ...), " "if undef, use input type",
    )
    argparser.add_argument(
        "-s",
        "--show_master_keys",
    ...
    argparser.add_argument(
        ...
        action="store_true",
        dest="ignore_mac",
        help="ignore Message Authentication Code " "during decryption",
    )
    argparser.add_argument(
        "--unencrypted-suffix",
        dest="unencrypted_suffix",
        help="override unencrypted key suffix "
        "(default: {default})".format(default=DEFAULT_UNENCRYPTED_SUFFIX),
    )
    argparser.add_argument(
        "--config",
        dest="config_loc",
        help="path to config file, disable recursive search"
        " (default: {default})".format(default=DEFAULT_CONFIG_FILE),
    )
    argparser.add_argument(
        "--encryption-context",
        dest="kmscontext",
        help="comma separated list of KMS encryption " "context key-value pairs",
    )
    argparser.add_argument(
    ...
    tree, need_key, existing_file = initialize_tree(
        args.file,
        itype,
        kms_arns=kms_arns,
        pgp_fps=pgp_fps,
    ...
    if not existing_file:
        ...
        if args.add_kms or args.add_pgp or args.rm_kms or args.rm_pgp:
            panic(
                "cannot add or remove keys on non-existent files, use "
                "`--kms` and `--pgp` instead.",
                error_code=49,
            )
    ...
    if existing_file:
        tree = walk_and_decrypt(tree, key, stash=stash, ignoreMac=args.ignore_mac)
    ...
    if args.set:
        ...
        while not valid_syntax or not has_master_keys:
            ...
            if args.show_master_keys:
                ...
                tree = load_file_into_tree(tmppath, otype, restore_sops=stash["sops"])
            if check_master_keys(tree):
                has_master_keys = True
            else:
                try:
                    print(
                        "ERROR: could not find a valid master key to encrypt"
        ...
        if tmphash == get_file_hash(tmppath):
            os.remove(tmppath)
            panic(
                "%s has not been modified, exit without writing" % args.file,
                error_code=200,
            )
...
def initialize_tree(
    path, itype, kms_arns=None, pgp_fps=None, configloc=None, kms_context=None
):
    ...
    if existing_file:
        ...
        tree, need_key = verify_or_create_sops_branch(
            tree,
            kms_arns=kms_arns,
            pgp_fps=pgp_fps,
            path=path,
        ...
        tree, need_key = verify_or_create_sops_branch(
            tree, kms_arns, pgp_fps, kms_context=kms_context
        )
...
def load_file_into_tree(path, filetype, restore_sops=None):
    ...
    with open(path, "rb") as fd:
        if filetype == "yaml":
            ...
            # tree['sops'] key.
            try:
                tree = json.loads(data.decode("utf-8"), object_pairs_hook=OrderedDict)
    ...
    for rule in config["creation_rules"]:
        ...
        if "filename_regex" in rule:
            if not re.search(rule["filename_regex"], filename):
                continue
        print(
            "INFO found a configuration for '%s' in '%s'" % (filename, configloc),
            file=sys.stderr,
        )
        return rule


def verify_or_create_sops_branch(
    tree, kms_arns=None, pgp_fps=None, path=None, configloc=None, kms_context=None
):
    ...
    if "sops" not in tree:
        tree["sops"] = dict()
        tree["sops"]["attention"] = (
            "This section contains key material"
    ...
    if "kms" in tree["sops"] and isinstance(tree["sops"]["kms"], MutableSequence):
    ...
    if "pgp" in tree["sops"] and isinstance(tree["sops"]["pgp"], MutableSequence):
    ...
    if not has_at_least_one_method:
        panic(
            "Error: No KMS ARN or PGP Fingerprint found to encrypt the data "
            "key, read the help (-h) for more information.",
            111,
        )
    return tree, need_new_data_key
...
def update_master_keys(tree, key):
    ...
    """
    if "kms" in tree["sops"]:
        if not isinstance(tree["sops"]["kms"], MutableSequence):
    ...
    if "pgp" in tree["sops"]:
        if not isinstance(tree["sops"]["pgp"], MutableSequence):
...
def walk_and_decrypt(
    ...
    for k, v in branch.items():
        ...
        if isinstance(v, MutableMapping):
            ...
            branch[k] = walk_list_and_decrypt(
                v,
                key,
                aad=caad,
                stash=nstash,
...
def walk_list_and_decrypt(
    branch, key, aad=b"", stash=None, digest=None, unencrypted=False
):
...
def decrypt(value, key, aad=b"", stash=None, digest=None, unencrypted=False):
    ...
    if A_is_newer_than_B(INPUT_VERSION, "0.8"):
        valtype = res.group(4)
    decryptor = Cipher(
        algorithms.AES(key), modes.GCM(iv, tag), default_backend()
    ).decryptor()
...
def walk_and_encrypt(
    branch, key, aad=b"", stash=None, isRoot=True, digest=None, unencrypted=False
):
    ...
    for k, v in branch.items():
        ...
        if isinstance(v, MutableMapping):
            ...
            branch[k] = walk_list_and_encrypt(
                v,
                key,
                aad=caad,
                stash=nstash,
    ...
    if isRoot:
        ...
        h = str()
        h = digest.hexdigest().upper()
        mac = encrypt(h, key, aad=branch["sops"]["lastmodified"].encode("utf-8"))
        branch["sops"]["mac"] = mac
    return branch


def walk_list_and_encrypt(
    branch, key, aad=b"", stash=None, digest=None, unencrypted=False
):
...
def encrypt(value, key, aad=b"", stash=None, digest=None, unencrypted=False):
    ...
    if stash and "cleartext" in stash and stash["cleartext"] == value:
        ...
        iv = os.urandom(32)
    encryptor = Cipher(
        algorithms.AES(key), modes.GCM(iv), default_backend()
    ).encryptor()
    encryptor.authenticate_additional_data(aad)
    enc_value = encryptor.update(value) + encryptor.finalize()
    return "ENC[AES256_GCM,data:{value},iv:{iv}," "tag:{tag},type:{valtype}]".format(
...
def get_key(tree, need_key=False):
    ...
    if need_key:
        ...
        print("INFO: generating and storing data encryption key", file=sys.stderr)
        key = os.urandom(32)
        if "kms" in tree["sops"]:
            i = -1
            for entry in tree["sops"]["kms"]:
                ...
                if updated is None:
                    panic(
                        "Failed to encrypt data key with KMS %s. "
        ...
        if "pgp" in tree["sops"]:
            ...
            for entry in tree["sops"]["pgp"]:
                ...
                if updated is None:
                    panic(
                        "Failed to encrypt data key with PGP %s. "
    ...
    if not (key is None):
        return key, tree
    panic("could not retrieve a key to encrypt/decrypt the tree", error_code=128)
...
def get_key_from_kms(tree):
    ...
    for entry in kms_tree:
        ...
        if "arn" not in entry or entry["arn"] == "":
            print("WARN: KMS ARN not found, skipping entry %s" % i, file=sys.stderr)
            continue
        kms, err = get_aws_session_for_entry(entry)
        if err != "":
            errors.append(
                "failed to obtain kms %s, error was: %s" % (entry["arn"], err)
            )
            continue
        if kms is None:
            errors.append("no kms client could be obtained for entry %s" % entry["arn"])
            continue
        context = entry["context"] if "context" in entry else {}
        if not isinstance(context, dict) and isinstance(context, MutableMapping):
            # ruamel.yaml's CommentedMap no longer subclasses dict as of
            # 0.15.52, but botocore only accepts dict subclasses, so unpack the
            # context as an unordered, uncommented dict.
            context = dict(**context)
        try:
            kms_response = kms.decrypt(
                CiphertextBlob=b64decode(enc), EncryptionContext=context
            )
        except Exception as e:
...
def encrypt_key_with_kms(key, entry):
    ...
    if kms is None or err != "":
        print(
            "ERROR: failed to initialize AWS KMS client for entry: %s" % err,
            file=sys.stderr,
        )
        return None
    context = entry["context"] if "context" in entry else {}
    try:
        kms_response = kms.encrypt(
            KeyId=entry["arn"], Plaintext=key, EncryptionContext=context
        )
    except Exception as e:
        print(
            "ERROR: failed to encrypt key using kms arn %s: %s" % (entry["arn"], e),
            file=sys.stderr,
        )
        return None
    entry["enc"] = b64encode(kms_response["CiphertextBlob"]).decode("utf-8")
...
def get_aws_session_for_entry(entry):
    ...
    try:
        ...
        return (None, "Unable to find region from ARN '%s' in entry" % entry["arn"])
    ...
    try:
        ...
        role = client.assume_role(
            RoleArn=entry["role"], RoleSessionName="sops@" + gethostname()
        )
    except Exception as e:
        return (None, "Unable to switch roles: %s" % e)
    try:
        print(
            "INFO: assuming AWS role '%s'" % role["AssumedRoleUser"]["Arn"],
            file=sys.stderr,
        )
...
def get_key_from_pgp(tree):
    ...
    for entry in pgp_tree:
        ...
        try:
            ...
            print(
                "INFO: PGP decryption failed in entry %s with error: %s" % (i, e),
                file=sys.stderr,
            )
            continue
        if len(key) == 32:
...
def encrypt_key_with_pgp(key, entry):
    ...
    try:
        ...
        print(
            "ERROR: failed to encrypt key using pgp fp %s: %s" % (fp, e),
            file=sys.stderr,
        )
        return None
    if p.returncode > 0:
...
def write_file(tree, path=None, filetype=None):
    ...
    if filetype == "yaml":
        if path == "stdout":
            sys.stdout.write(
                ruamel.yaml.dump(tree, Dumper=ruamel.yaml.RoundTripDumper, indent=4)
            )
        else:
            fd.write(
                ruamel.yaml.dump(
                    tree, Dumper=ruamel.yaml.RoundTripDumper, indent=4
                ).encode("utf-8")
            )
    elif filetype == "json":
        ...
        if "data" in tree:
            ...
            if path == "stdout":
                if sys.version_info[0] == 3 and isinstance(tree["data"], bytes):
...
def run_editor(path):
    ...
    if "EDITOR" in os.environ:
        ...
        process = subprocess.Popen(
            ["which", "vim", "nano"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def check_rotation_needed(tree):
    ...
    if "kms" in tree["sops"]:
        for entry in tree["sops"]["kms"]:
            ...
            if "created_at" in entry:
                d = datetime.strptime(entry["created_at"], "%Y-%m-%dT%H:%M:%SZ")
                if d < six_months_ago:
    ...
    if show_rotation_warning:
        print(
            "INFO: the data key on this document is over 6 months old. "
...
def check_latest_version():
    print(
        "INFO: You're using Sops 1 written in Python."
    ...
    try:
        ...
        if A_is_newer_than_B(latest, VERSION):
            if platform.system() == "Darwin":
                print(
                    "INFO: to update to sops " + latest + ", use:"

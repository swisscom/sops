...
class TreeTest(unittest2.TestCase):
    ...
    def test_verify_or_create_sops_branch(self):
        ...
        kms_arns = (
            "arn:aws:kms:us-east-1:656532927350:key/"
        ...
        tree = OrderedDict()
        tree, ign = sops.verify_or_create_sops_branch(
            tree, kms_arns=kms_arns, pgp_fps=pgp_fps, kms_context=kms_context
        )
        log.debug("%s", tree)
        ...
        assert (
            tree["sops"]["kms"][0]["role"] == "arn:aws:iam::927034868273:role/sops-dev"
        )
        ...
        assert (
            tree["sops"]["pgp"][0]["fp"] == "85D77543B3D624B63CEA9E6DBC17301B491B3F21"
        )
        assert (
            tree["sops"]["pgp"][1]["fp"] == "C9CAB0AF1165060DB58D6D6B2653B624D620786D"
        )
    ...
    def test_walk_and_encrypt_unencrypted(self):
        ...
        tree_mac = sops.decrypt(
            tree["sops"]["mac"], key, aad=tree["sops"]["lastmodified"].encode("utf-8")
        )
    ...
    def test_add_kms_master_keys(self):
        ...
        assert (
            tree["sops"]["kms"][1]["arn"]
            == "arn:aws:kms:us-east-1:656532927350:key/920abb2e-c2b3-9090-943a-047fa387f3ac"
        )
        assert (
            tree["sops"]["kms"][1]["role"]
    ...
    def test_add_pgp_master_keys_where_none_existed(self):
        ...
        assert (
            tree["sops"]["pgp"][0]["fp"] == "E60892BB9BD89A69F759A1A0A3D652173B763E8F"
        )
    ...
    def test_add_pgp_master_keys(self):
        ...
        assert (
            tree["sops"]["pgp"][1]["fp"] == "E60892BB9BD89A69F759A1A0A3D652173B763E8F"
        )
